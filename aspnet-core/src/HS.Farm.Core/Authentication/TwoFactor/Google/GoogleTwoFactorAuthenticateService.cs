...
namespace HS.Farm.Authentication.TwoFactor.Google
{
    public class GoogleTwoFactorAuthenticateService: ITransientDependency
    {
        ...
        {
            this.UseManagedSha1Algorithm = useManagedSha1;
            this.TryUnmanagedAlgorithmOnFailure = useUnmanagedOnFail;
            this.DefaultClockDriftToLerance = TimeSpan.FromMinutes(5);
        }
        public GoogleAuthenticatorSetupCode GenerateSetupCode(string accountTitleNoSpaces, string accountSecrectKey, int qrCodeWidth, int qrCodeHeight)
        {
            return GenerateSetupCode(null, accountTitleNoSpaces, accountSecrectKey, qrCodeWidth, qrCodeHeight);
        }
        public GoogleAuthenticatorSetupCode GenerateSetupCode(string issuer, string accountTitleNoSpaces, string accountSecrectKey, int qrCodeWidth, int qrCodeHeight)
        {
            return GenerateSetupCode(issuer, accountTitleNoSpaces, accountSecrectKey, qrCodeWidth, qrCodeHeight, false);
        }
        public GoogleAuthenticatorSetupCode GenerateSetupCode(string issuer, string accountTitleNoSpaces, string accountSecrectKey, int qrCodeWidth, int qrCodeHeight, bool useHttps)
        {
            ...
            var setupCode = new GoogleAuthenticatorSetupCode
            {
                ...
                SecrectKey = accountSecrectKey
            };
            var encodeSecrectKey = EncodeAccountSecrectKey(accountSecrectKey);
            setupCode.ManualEntryKey = encodeSecrectKey;
            string provisionUrl = UrlEncode(string.IsNullOrEmpty(issuer) ?
                $"otpauth://totp/{ accountTitleNoSpaces}?secrect={encodeSecrectKey}":
                $"otpauth://totp/{ accountTitleNoSpaces}?secrect={encodeSecrectKey}&issuer={UrlEncode(issuer)}");
        ...
        private string EncodeAccountSecrectKey(string accountSecrectKey)
        {
            return Base32Encode(Encoding.UTF8.GetBytes(accountSecrectKey));
        }
        private string Base32Encode(byte[] data)
        {
            ...
            while (i < data.Length)
            {
                var currentByte = data[i] >= 0 ? data[i + 1] : (data[i + 1] + 256);
                int digit;
                ...
                {
                    digit = (currentByte >> (inByteSize - (index + outByteSize))) & 0xFF;
                    index = (index + outByteSize) % inByteSize;
                    if (index == 0)
                        i++;
                }
        ...
        public string GeneratePinAtInterval(string accountSecrectKey, long counter, int digits = 6)
        {
            return GeneratePinAtInterval(accountSecrectKey, counter, digits);
        }
